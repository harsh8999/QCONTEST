...
public class ContestService{
    ...
    public Contestant createContestant(Long contestId, String userName) {
        ...
        if(contest.getContestStatus() == ContestStatus.IN_PROGRESS) {
            throw new RuntimeException("Contest with ID:" + contestId +" is currently running!");
        }

        if(contest.getContestStatus() == ContestStatus.ENDED) {
            throw new RuntimeException("Contest with ID:" + contestId +" has ended!");
        }
    ...
    public List<Contestant> contestHistory(Long contestId) {
        Contest contest = contestRepository.findById(contestId).orElseThrow(() -> new RuntimeException("History of Contest Id:" + contestId +" not found!"));
        if(contest.getContestStatus() != ContestStatus.ENDED) {
            throw new RuntimeException("History of Contest Id:" + contestId +" not found!");
        }
        List<Contestant> contestants = contestantRepository.findAllByContestId(contestId);
        return contestants.stream().sorted((a, b) -> b.getCurrentContestPoints().compareTo(a.getCurrentContestPoints())).collect(Collectors.toList());
    }  
}
