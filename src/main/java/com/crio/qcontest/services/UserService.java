...
public class UserService{
    ...
    public List<User> getUsers(UserOrder userOrder) {
        ...
        if(userOrder == UserOrder.SCORE_ASC) {
            return users.stream().sorted((a, b) -> a.getTotalScore().compareTo(b.getTotalScore())).collect(Collectors.toList());
        }
        return users.stream().sorted((a, b) -> b.getTotalScore().compareTo(a.getTotalScore())).collect(Collectors.toList());
    } 
}
